...
- **Snowflake auth** (`snowflake_client.py`): Auth via `SNOWFLAKE_AUTH_METHOD` env var — key pair (RSA `.p8` file, default), Azure AD OAuth (MSAL client_credentials), or password (e.g. via Key Vault references on Azure). Caches a global connection and auto-reconnects on failure.
